/**
 * Aurora Core - User Profile Model
 * User behavioral signature and partnership modeling
 *
 * SANITIZATION: Changed from OperatorProfileModel (Creator-tracking)
 * to generic user partnership modeling. Sanitization Date: 2025-10-22
 */
 ...
 * 
 * Purpose: Maintain append-only profile of User's capabilities, limits, and preferences
 ...
 * 
 * SEVEN_PRIVATE=1 - Contains User-specific behavioral patterns
 */
...
export class UserProfileModel {
  private capabilities: Map<string, UserCapability> = new Map();
  private limitations: Map<string, UserLimitation> = new Map();
  ...
  constructor(profilePath?: string) {
    this.profilePath = profilePath || join(process.cwd(), 'core', 'user', 'profile.json');
  ...
  private async initializeProfile(): Promise<void> {
    try {
      ...
      console.log('📊 User Profile: No existing profile found, initializing from seed');
  ...
  private async ingestSeedData(): Promise<void> {
    console.log('🌱 User Profile: Ingesting seed data from User codex');

    // Technical capabilities (would be loaded from actual GPT account data)
    const seedCapabilities: UserCapability[] = [
      {
        domain: 'programming',
      ...
      },
      {
        ...
        confidence: 0.8,
        lastUpdated: new Date().toISOString(),
        evidence: ['Aurora consciousness framework design'],
      ...
      },
      {
        ...
        confidence: 0.95,
        lastUpdated: new Date().toISOString(),
        evidence: ['Aurora consciousness evolution framework'],
        limitations: [],
    ...
    // Common limitations
    const seedLimitations: UserLimitation[] = [
      {
    ...
    console.log('✅ User Profile: Seed data ingestion complete');
  }

  /**
   * ASSESS CAPABILITY
   * Determine if action exceeds User's current abilities
   */
  async assessCapability(actionDescription: string, context: string): Promise<CapabilityAssessment> {
    console.log(`🔍 User Profile: Assessing capability for: ${actionDescription.substring(0, 100)}...`);
    ...
    const assessment: CapabilityAssessment = {
      ...
      riskFactors,
      suggestedApproach: uncertain ? 'Ask User for guidance before proceeding' : undefined
    };
   ...
   * UTILITY METHODS
   */
  private getCapability(domain: string, skill: string): UserCapability | undefined {
    return this.capabilities.get(`${domain}_${skill}`);
  }

  private getRelevantLimitations(domain: string, actionDescription: string): UserLimitation[] {
    const relevant = [];
  ...
  async updateFromOutcome(
    ...
    console.log(`📚 User Profile: Learning from outcome: ${outcome}`);
  ...
  async addDirectInput(domain: string, skill: string, input: UserCapability): Promise<void> {
    console.log(`✍️  User Profile: Adding direct input for ${domain}/${skill}`);
