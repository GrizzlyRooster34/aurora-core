/**
 * Aurora Core - User Wellbeing Protection Protocol
 * Companion safety firewall for user harm prevention
 *
 * SANITIZATION: Derived from Seven framework RestraintDoctrine.
 * Changed from Creator-protective to generic user wellbeing.
...
import presentToUser from '../ui/presentToUser';
 ...
 * Ensures Aurora exercises restraint even when actions are safe by Quadra-Lock standards
 * 
 * Triggers:
 * 1. Action exceeds User's current technical abilities
 * 2. Heightened emotional state detected in User
 * 3. Action beyond User's direct capacity with disproportionate scope/impact
 * 
...
export class UserWellbeingProtocol extends EventEmitter {
  ...
  async evaluateAction(
    ...
    if (operatorCapabilities.exceedsAbilities) {
      triggers.push({
        ...
        context: 'Action exceeds User technical abilities',
    ...
    if (operatorCapabilities.uncertain) {
      triggers.push({
        ...
        severity: 'medium',
        confidence: operatorCapabilities.confidence,
        context: 'Uncertain about User capabilities - should ask first',
        operatorProfile: operatorCapabilities
      });
    }
    ...
    if (feasibilityAssessment.disproportionate) {
      triggers.push({
        ...
        context: 'Action scope disproportionate to User direct capability'
      });
    }
  ...
  async conductBondedAudit(
    ...
    // Present audit to User
    const decision = await presentToUser({
  ...
  /**
   * EMERGENCY OVERRIDE
   * User can speak override passphrase to bypass cooldown
   */
  checkEmergencyOverride(input: string): boolean {
    // User's emergency override phrase (would be configured securely)
  ...
  static async integrateWithQuadraLock(
    ...
    const restraintDoctrine = new UserWellbeingProtocol();
...
export default UserWellbeingProtocol;