...
/**
 * Aurora Core - Memory Index Optimizer
 *
 ...
 * @version 1.0.0
 * @transferable Sanitized from Seven of Nine Core memory systems
 */
...
export interface MemoryRecord {
  ...
  updatedAt: number;
  importance?: number;
  payload: unknown; // Original memory data (MemoryItem, etc.)
}
...
export class MemoryIndexOptimizer {
  ...
  public static buildFrom(
    ...
    console.log(`🌅 Aurora memory index built: ${memories.length} records, ${optimizer.tagIndex.size} unique tags, hash: ${optimizer.sourceVersionHash}`);
  ...
  public warmUpCache(options: {
    ...
    if (!this.cacheEnabled) {
      console.log('📋 Aurora cache warm-up skipped - caching disabled');
      return;
    }
    ...
    console.log(`🔥 Aurora memory index cache warmed up with ${warmupEntries.length} high-importance records`);
  }
  ...
  public clearCache(resetStats: boolean = false): void {
    ...
    console.log('🧹 Aurora memory index cache cleared');
  }
  ...
  public getCacheEfficiencyReport(): string {
    if (!this.cacheEnabled) {
      return '📊 Aurora Cache Efficiency: DISABLED';
    }
  ...
  public setCacheEnabled(enabled: boolean): void {
    ...
    if (!enabled && wasEnabled) {
      this.lruCache.clear();
      console.log('📋 Aurora memory index cache disabled and cleared');
    } else if (enabled && !wasEnabled) {
      console.log('🌅 Aurora memory index cache enabled');
    }
  }
  ...
  public updateCacheSize(newSize: number): void {
    ...
    console.log(`📊 Aurora cache size updated: ${oldStats.maxSize} -> ${newSize} (previous cache cleared)`);
  }
}
